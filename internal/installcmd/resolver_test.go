...
func TestResolveAgentInstall(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    CommandSequence{{"npm", "install", "-g", "--ignore-scripts", "@anthropic-ai/claude-code@2.1.140"}},
		},
		{
			name:    "claude-code on linux system npm uses sudo",
			profile: system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroUbuntu, PackageManager: "apt"},
			agent:   model.AgentClaudeCode,
			want:    CommandSequence{{"sudo", "npm", "install", "-g", "--ignore-scripts", "@anthropic-ai/claude-code@2.1.140"}},
		},
		{
		...
		},
		{
			...
			profile: system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroUbuntu, PackageManager: "apt"},
			agent:   model.AgentOpenCode,
			want:    CommandSequence{{"sudo", "npm", "install", "-g", "--ignore-scripts", "opencode-ai@1.14.48"}},
		},
		{
			name:    "opencode on ubuntu nvm skips sudo",
			profile: system.PlatformProfile{OS: "linux", LinuxDistro: system.LinuxDistroUbuntu, PackageManager: "apt", NpmWritable: true},
			agent:   model.AgentOpenCode,
			want:    CommandSequence{{"npm", "install", "-g", "--ignore-scripts", "opencode-ai@1.14.48"}},
		},
		{
