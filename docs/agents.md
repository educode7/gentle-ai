...
- **MCP adapter wiring**: ComponentEngram declares `npm:pi-mcp-adapter` `2.5.4` in `.pi/settings.json`, adds `pi-mcp-adapter` `^2.5.4` to `.pi/npm/package.json`, and activates `.pi/mcp.json` with `engram mcp --tools=agent` plus `directTools: true` without removing unrelated user entries.
